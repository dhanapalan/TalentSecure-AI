...
test.describe('CRUD — Colleges (delete is a suspend alias)', () => {
  ...
  test('Create — valid college succeeds', async ({ request }) => {
    const res = await request.post(`${API_URL}/superadmin/colleges`, {
      ...
      data: {
        ...
        addressLine1: '1 QA Road',
        city: 'Chennai',
        district: 'Chennai',
        state: 'Tamil Nadu',
        country: 'India',
        pincode: '600001',
        website: `https://qa-crud-${suffix}.edu`,
  ...
  test('Create — duplicate email rejected (409)', async ({ request }) => {
    const res = await request.post(`${API_URL}/superadmin/colleges`, {
      headers: auth(token),
      data: {
        name: `Dup College ${suffix}`,
        establishmentYear: 1999,
        institutionType: 'University',
        ownership: 'Private',
        categories: ['Management'],
        addressLine1: '2 QA Road',
        city: 'Chennai',
        district: 'Chennai',
        state: 'Tamil Nadu',
        pincode: '600002',
        website: 'https://dup-college.edu',
        email: `qa-crud-college-${suffix}@college.edu`,
        phone: '+919876543211',
        tpoName: 'X',
        tpoEmail: `tpo-dup-${suffix}@college.edu`,
      },
    });
    expect(res.status()).toBe(409);
  });
